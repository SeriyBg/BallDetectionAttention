...
def _iou(boxA, boxB):
    boxA = _to_x1y1x2y2(boxA)
    boxB = _to_x1y1x2y2(boxB)
...
def _player_detection_stats(pred_boxes, gt_boxes, iou_thresh):
    tp = 0
    fp = 0
    fn = 0
...
def _player_detection_stats_by_center(pred_boxes, gt_boxes, tolerance):
    tp = 0
    fp = 0
    fn = 0

    pred_centers = [_get_box_center(box) for box in pred_boxes]
    gt_centers = [_get_box_center(box) for box in gt_boxes]
...
def evaluate_player_detection_results(annotations, gt_annotations, iou_thresh):
    frame_stats = []

    start_frame = min(min(gt_annotations.persons), min(annotations.persons))
    end_frame = max(max(gt_annotations.persons), max(annotations.persons))

    for i in range(start_frame, end_frame):
        pred_boxes = annotations.persons[i]
        gt_boxes = gt_annotations.persons[i]
        # stats = _player_detection_stats(pred_boxes, gt_boxes, iou_thresh)
        stats = _player_detection_stats_by_center(pred_boxes, gt_boxes, iou_thresh)
        (_, _, correctly_classified) = stats
        if not correctly_classified:
            print(f'Frame: {i}; GT: {gt_boxes}; Detected: {pred_boxes}')
        frame_stats.append(stats)

    percent_correctly_classified_frames = sum([c for (_, _, c) in frame_stats]) / len(frame_stats)

    temp = [p for (p, _, _) in frame_stats if p is not None]
    avg_precision = sum(temp) / len(temp) if temp else 0

    temp = [r for (_, r, _) in frame_stats if r is not None]
    avg_recall = sum(temp) / len(temp) if temp else 0

    return avg_precision, avg_recall, percent_correctly_classified_frames


def visualize_detection_results(camera_id, dataset_path, gt_annotations=None, annotations=None, frames=[]):
    '''
    ...
    count_frames = -1
    while (sequence.isOpened() and count_frames < 10):
        ret, frame = sequence.read()
        count_frames += 1

        if count_frames not in frames:
            continue
        if not ret:
            # End of sequence
            break
...
def save_detection_results(camera_id, dataset_path, gt_annotations=None, annotations=None, frames=[]):
    '''
    Visualize ground truth annotations (in blue) and detected annotations (in red)
    :param camera_id: ID of the ISSIA video sequence
    :param gt_annotations: SequenceAnnotations object with ground truth annotations
    ...
    while sequence.isOpened():
        ret, frame = sequence.read()
        count_frames += 1
        if len(frames) > 0 and count_frames not in frames:
            continue
